...
phone-code-expired-title = 验证码过期或不正确，请再试一次。
...
account-post-not-before-title=还没到下次可发帖的时间哦
account-post-not-before-detail=下次发帖时间: {$not_before} 之后
...
time-cursor=帖子曝光时间
min-length-error-title=至少要{$length}个字符哦
min-length-error-detail=当前字符：{$current_length}，请完善后再提交吧～
max-length-error-title=最多{$length}个字符哦
max-length-error-detail=当前字符：{$current_length}，请修改再提交
content-sensitive-title=包含敏感词汇
content-sensitive-detail=请修改后再提交