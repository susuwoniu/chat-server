...
async fn main() -> Result<(), error::Error> {
    ...
    match opt {
        ...
        Opt::Server {} => {
            ...
            let refresh_token_secret_key = cfg.auth.refresh_token_secret_key.clone();
            let refresh_token_public_key = cfg.auth.refresh_token_public_key.clone();
            if refresh_token_secret_key.is_empty() || refresh_token_public_key.is_empty() {
                panic!("Missing required config for auth.refresh_token_secret_key or auth.refresh_token_public_key");
            }
            ...
            let server = HttpServer::new(move || {
                ...
                let refresh_token_pairs = RefreshTokenPair(Pair::from_string(
                    refresh_token_secret_key.clone(),
                    refresh_token_public_key.clone(),
                ));
                ...
                App::new()
                    ...
                    .app_data(web::Data::new(refresh_token_pairs))
