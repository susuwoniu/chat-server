    ...
    async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
            ...
            TypedHeader::<Authorization<Bearer>>::from_request(req)
                ...
                .map_err(|err| {
                    ServiceError::unauthorized(
                        &locale,
                        "parse_header_authorization_failed",
                        err.into(),
                    )
                })?;
        let TypedHeader(XClientId(x_client_id)) =
            TypedHeader::<XClientId>::from_request(req).await?;
        ...
        match verify_result {
            Ok(result) => {
                ...
                if let (Some(sub), Some(client_id), Some(roles), Some(jti)) =
                    (sub_option, client_id_option, roles_option, jti_option)
                {
